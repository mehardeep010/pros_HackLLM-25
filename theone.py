...
# Setup device and global models
...
class FidelityGraphDataset(PyGDataset):
    ...
    def get(self, idx):
        ...
        # Retry logic if graph building fails
...
class SFG_GNN(Module):
    ...
    def forward(self, data):
        ...
        for i in range(data.num_graphs):
            ...
            ref_idx = hyp_idx + 1
            if hyp_idx < e and ref_idx < e:
                hyp.append(x[hyp_idx]); ref.append(x[ref_idx])
                
        if not hyp: return torch.tensor([]), torch.tensor([])
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        for batch_idx, batch in enumerate(pbar):
            try:
                ...
                if pos_scores.nelement() == 0: continue
